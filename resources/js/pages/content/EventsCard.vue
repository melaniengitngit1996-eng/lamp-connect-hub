...
<template>
    <div class="rounded-xl border bg-card text-card-foreground shadow p-0">
        <div class="flex items-center justify-between p-4 border-b gap-3">
            <div class="min-w-0" data-tsd-source="/src/routes/_app.admin.content.tsx:122:9">
                <div class="text-sm font-medium" data-tsd-source="/src/routes/_app.admin.content.tsx:123:11">Highlights</div>
                <div class="text-xs text-muted-foreground truncate" data-tsd-source="/src/routes/_app.admin.content.tsx:124:11">Gatherings, conferences, and camps. · {{ events.length }} {{ events.length === 1 ? 'entry' : 'entries' }}</div>
            </div>
            <button v-if="can('content.create')" @click="newEvent" class="inline-flex items-center justify-center gap-2 whitespace-nowrap font-medium cursor-pointer transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground shadow hover:bg-primary/90 h-8 rounded-md px-3 text-xs" data-tsd-source="/src/routes/_app.admin.content.tsx:126:9">
            ...
            <table class="w-full caption-bottom text-sm">
                <thead class="[&amp;_tr]:border-b">
                    <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                        ...
                        <th class="h-10 px-2 text-left align-middle font-medium text-muted-foreground [&amp;:has([role=checkbox])]:pr-0 [&amp;&gt;[role=checkbox]]:translate-y-[2px]">Venue</th>
    ...
    <ConfirmDialog
        ...
        :message="`Delete '${selectedEvent?.title}'? This action cannot be undone.`"
        confirm-text="Delete"
        :loading="deleting"
