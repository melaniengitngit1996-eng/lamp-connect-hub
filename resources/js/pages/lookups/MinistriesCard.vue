...
<template>
<div data-state="active" data-orientation="horizontal" role="tabpanel" id="radix-_r_u_-content-ministries" tabindex="0" class="ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 mt-4" data-tsd-source="/src/routes/_app.admin.lookups.tsx:62:9">
	<div class="rounded-xl border bg-card text-card-foreground shadow p-0">
		<div class="flex items-center justify-between p-4 border-b gap-3">
			<div class="flex items-center gap-3">
				<div class="text-sm text-muted-foreground">{{ ministries.length }} entr{{ ministries.length === 1 ? 'y' : 'ies' }}</div>
				<select
			...
			<table class="w-full caption-bottom text-sm">
				<thead class="[&amp;_tr]:border-b">
					<tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
						...
						<th class="h-10 px-2 text-left align-middle font-medium text-muted-foreground [&amp;:has([role=checkbox])]:pr-0 [&amp;&gt;[role=checkbox]]:translate-y-[2px]">Local Church</th>
...
<ConfirmDialog
    ...
    :message="`Delete '${selectedMinistry?.name}'? This action cannot be undone.`"
    confirm-text="Delete"
    :loading="deleting"
